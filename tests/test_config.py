...
class TestGetConfig:
    ...
    def test_env_var_overrides(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setenv("BOMI_MY_SETTING", "from-env")
...
class TestFindProjectDir:
    def test_explicit_override(self, tmp_path):
        (tmp_path / ".bomi").mkdir()
        (tmp_path / ".bomi" / "project.yaml").write_text("name: test\n")
    ...
    def test_env_var(self, monkeypatch, tmp_path):
        (tmp_path / ".bomi").mkdir()
        (tmp_path / ".bomi" / "project.yaml").write_text("name: test\n")
        monkeypatch.setenv("BOMI_PROJECT", str(tmp_path))
